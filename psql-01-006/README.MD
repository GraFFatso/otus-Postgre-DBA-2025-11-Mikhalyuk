...
ошибка из-за того, что в имени кластера существует пробел
кластер пересоздал с именем task_001_006
...
```
прерывалось с ошибкой, т.к. тербовался пароль пользователя СУБД, задал пароль
```
...
менял значения  параметров shared_buffers, effective_cahe_size, effective_io_concurrency, work_mem, maintenance_work_mem \
...
выключение fsync не привело к значительному изменению показателя tps
...
с ключом -r увидел детализацию, видно что UPDATE занимает больше всего времени
```
...
```
создал доп. скрипт для для pgbench, чтобы обеспечить переполнение памяти выборками данных из существующих таблиц
```
...
```
т.к. 'connections' у кластера = 153, то нагрузку создавал в количестве 128 соединений


############################################################### \
почитал про ключ -j pgbench..... \
64 - Это значение избыточно, установлю в 2 или 4, и буду смотреть на показатели \
64 - при таком значении львиную дол.ю процессорных мощностей использует сама утилита создания нагрузки, СУБД остается мало ЦП, поэтому я не видел разницы в показателях \
т.к. при установке -j 5 и sync_commit =off при прочих условиях pgbench показал в районе  2000 tps

утилита atop показывает  загруженность "диска" 
